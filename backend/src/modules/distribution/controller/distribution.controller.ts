...
export class DistributionController {
  constructor(private readonly service: DistributionService = new DistributionService()) {}

  register(app: FastifyInstance): void {
    const readGuard = [app.authenticate, app.requireTenantRead];
    const writeGuard = [app.authenticate, app.requireTenantScope];

    app.get('/distribution/meta', { preHandler: readGuard }, async (req, reply) => {
      reply.send(ok(await this.service.getMeta(req)));
    });

    app.get('/distribution', { preHandler: readGuard }, async (req, reply) => {
    ...
    app.get('/distribution/:id', { preHandler: readGuard }, async (req, reply) => {
      const { id } = req.params as { id: string };
      reply.send(ok(await this.service.getById(req, id)));
    });

    app.get('/distribution/:id/history', { preHandler: readGuard }, async (req, reply) => {
      const { id } = req.params as { id: string };
      reply.send(ok(await this.service.history(req, id)));
    });

    app.post('/distribution', { preHandler: writeGuard }, async (req, reply) => {
      const row = await this.service.createDraft(req, (req.body ?? {}) as Record<string, unknown>);
      await app.auditHook(req, 'create', 'distribution', row.id);
      reply.code(201).send(ok(row));
    });

    app.patch('/distribution/:id', { preHandler: writeGuard }, async (req, reply) => {
      const { id } = req.params as { id: string };
      const row = await this.service.updateDraft(req, id, (req.body ?? {}) as Record<string, unknown>);
      await app.auditHook(req, 'update', 'distribution', id);
      reply.send(ok(row));
    });

    app.post('/distribution/:id/submit', { preHandler: writeGuard }, async (req, reply) => {
      const { id } = req.params as { id: string };
      const row = await this.service.submit(app, req, id);
      reply.send(ok(row));
    });

    app.post('/distribution/:id/review', { preHandler: writeGuard }, async (req, reply) => {
      const { id } = req.params as { id: string };
      const row = await this.service.review(app, req, id, (req.body ?? {}) as Record<string, unknown>);
      reply.send(ok(row));
    });

    app.post('/distribution/:id/correct', { preHandler: writeGuard }, async (req, reply) => {
      const { id } = req.params as { id: string };
      const row = await this.service.correct(req, id, (req.body ?? {}) as Record<string, unknown>);
      await app.auditHook(req, 'correct', 'distribution', id);
      reply.send(ok(row));
    });

    app.get('/distribution/export/csv', { preHandler: readGuard }, async (req, reply) => {
      const csv = await this.service.exportCsv(req);
      reply.header('Content-Type', 'text/csv; charset=utf-8');
      reply.header('Content-Disposition', 'attachment; filename="distribution-register.csv"');
      reply.send(csv);
    });

    app.get('/distribution/export/xlsx', { preHandler: readGuard }, async (req, reply) => {
    ...
    app.get('/distribution/export/pdf', { preHandler: readGuard }, async (req, reply) => {
