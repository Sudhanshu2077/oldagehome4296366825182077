...
- [x] **Frontend UI refresh + preview sample data** (frontend-only): shared UI kit `src/components/ui.tsx` (ScreenHeader, PrimaryButton, GhostButton, Card, Chip, StatusBadge, SampleBadge, SampleBanner, EmptyState, KpiCard, SectionTitle, KeyValueRow, Scrollable); tab redesigns to match the modern registers-tab style — `reports.tsx` (report-tile grid + record-card detail view), `modules.tsx` and `hr.tsx` (2-col monogram card grids with subtitles). **Preview sample data system** (`src/sample/sampleData.ts` + `src/sample/SampleContext.tsx`): 2–3 client-side-only test rows per data-filling tab/list (announcements, events, inquiries, health + vitals, finance income/expense/voucher/donation/budget/cash/bank, reports, all 9 register lists, and generic `module/[code].tsx` via `sampleRowsForModule`), shown ONLY when the real list is empty, tagged with `__sample` + SampleBadge + SampleBanner, NEVER written to the backend, gated by a persisted toggle (tokenStorage `showSamples`, default ON) with an off-switch in Profile → Settings (`settings.showSamples`). To remove before launch: flip the toggle, or delete `src/sample/`. Government/portal/dashboard KPIs intentionally excluded (server-computed aggregates would be misleading).
