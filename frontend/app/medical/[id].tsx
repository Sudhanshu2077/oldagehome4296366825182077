...
const FIELD_LABELS: Record<string, string> = {
  personName: 'med.studentName',
  diseaseNature: 'med.diseaseNature',
  illnessDate: 'med.illnessDate',
  medicineParticulars: 'med.medicineParticulars',
  medicineAllowances: 'med.medicineAllowances',
  medicalOfficerName: 'med.medicalOfficer',
  medicalOfficerSignature: 'med.medicalOfficer',
  remarks: 'med.remarks',
};
...
export default function MedicalDetailScreen() {
  ...
  const { t } = useI18n();
  const [row, setRow] = useState<MedicalEntry | null>(null);
  ...
  const styles = useMemo(() => StyleSheet.create({
    ...
    sourceFlag: { fontSize: 10, color: palette.warning, marginBottom: spacing.sm },
  ...
  const load = useCallback(async () => {
    try {
      ...
      setRow((res.data as { data: MedicalEntry }).data);
      setError(null);
    } catch (err) {
  ...
  async function review() {
    ...
    setError(null);
    try {
      await api.post(`/medical/${id}/review`);
      await load();
    } catch (err) {
      setError(errorMessage(err));
    } finally {
  ...
  async function correct() {
    ...
    setError(null);
    try {
      await api.post(`/medical/${id}/correct`, { field: corrField, value: corrValue, reason: corrReason });
    ...
    <View style={styles.container}>
      <ScrollView style={styles.scroll}>
        ...
        <View style={styles.card}>
          <View style={styles.cardTop}>
            ...
            <View style={[styles.statusBadge, { backgroundColor: STATUS_COLORS[row.status] ?? '#6b7280' }]}>
              <Text style={styles.statusText}>{t(`med.status${row.status}`)}</Text>
            </View>
          </View>
          <View style={styles.fieldRow}><Text style={styles.fieldLabel}>{t('med.illnessDate')}</Text><Text style={styles.fieldValue}>{row.illnessDate ? String(row.illnessDate).slice(0, 10) : '—'}</Text></View>
          <View style={styles.fieldRow}><Text style={styles.fieldLabel}>{t('med.studentName')}</Text><Text style={styles.fieldValue}>{row.personName || '—'}</Text></View>
          <View style={styles.fieldRow}><Text style={styles.fieldLabel}>{t('med.diseaseNature')}</Text><Text style={styles.fieldValue}>{row.diseaseNature || '—'}</Text></View>
          <View style={styles.fieldRow}><Text style={styles.fieldLabel}>{t('med.medicineParticulars')}</Text><Text style={styles.fieldValue}>{row.medicineParticulars || '—'}</Text></View>
          <View>
            <View style={styles.fieldRow}><Text style={styles.fieldLabel}>{t('med.medicineAllowances')}</Text><Text style={styles.fieldValue}>{row.medicineAllowances || '—'}</Text></View>
            <Text style={styles.sourceFlag}>{t('med.sourceFlag')}</Text>
          </View>
          <View style={styles.fieldRow}><Text style={styles.fieldLabel}>{t('med.medicalOfficer')}</Text><Text style={styles.fieldValue}>{row.medicalOfficerName || '—'} {row.medicalOfficerSignature ? `| ${row.medicalOfficerSignature}` : ''}</Text></View>
          <View style={styles.fieldRow}><Text style={styles.fieldLabel}>{t('med.remarks')}</Text><Text style={styles.fieldValue}>{row.remarks || '—'}</Text></View>
          <View style={styles.fieldRow}><Text style={styles.fieldLabel}>{t('med.submittedAt')}</Text><Text style={styles.fieldValue}>{row.submittedAt ? String(row.submittedAt).slice(0, 19) : '—'}</Text></View>
          <View style={styles.fieldRow}><Text style={styles.fieldLabel}>{t('med.finalizedAt')}</Text><Text style={styles.fieldValue}>{row.finalizedAt ? String(row.finalizedAt).slice(0, 19) : '—'}</Text></View>
        </View>

        {canWrite && row.status === 'DRAFT' ? (
          <View style={styles.actionsRow}>
            <TouchableOpacity style={[styles.actionBtn, styles.actionPrimary]} onPress={() => void submit()} disabled={busy}>
              <Text style={styles.actionText}>{t('med.submit')}</Text>
            </TouchableOpacity>
          </View>
        ) : null}
        ...
        {canReview && row.status === 'SUBMITTED' ? (
          <View style={styles.actionsRow}>
            <TouchableOpacity style={[styles.actionBtn, styles.actionPrimary]} onPress={() => void review()} disabled={busy}>
              <Text style={styles.actionText}>{t('med.finalize')}</Text>
            </TouchableOpacity>
          </View>
        ) : null}
        ...
        {canReview && row.status === 'FINALIZED' ? (
          <View>
            <Text style={styles.note}>{t('med.correctionWorkflow')}</Text>
            <View style={styles.actionsRow}>
              <TouchableOpacity style={[styles.actionBtn, { backgroundColor: palette.surfaceAlt }]} onPress={() => setCorrectionModal(true)}>
                <Text style={{ color: palette.text, fontWeight: '600' }}>{t('med.edit')}</Text>
              </TouchableOpacity>
            </View>
          </View>
        ) : null}
        ...
        {row.changes.length > 0 ? (
          <View style={styles.card}>
            <Text style={styles.sectionTitle}>{t('med.auditHistory')}</Text>
            {row.changes.map((c, i) => (
              <View key={i} style={styles.auditRow}>
                <Text style={styles.fieldValue}>{t(FIELD_LABELS[c.field] ?? c.field)}</Text>
                <Text style={styles.note}>{t('med.previousValue')}: {String(c.previousValue ?? '')} → {t('med.newValue')}: {String(c.newValue ?? '')}</Text>
                <Text style={styles.note}>{t('med.reasonForChange')}: {c.reason}</Text>
      ...
      <Modal visible={correctionModal} animationType="slide" transparent>
        <View style={styles.modalBackdrop}>
          <View style={styles.modalCard}>
            <Text style={styles.modalTitle}>{t('med.edit')}</Text>
            <Text style={styles.label}>{t('med.correctionWorkflow')}</Text>
            ...
            <Text style={styles.label}>{t('med.newValue')}</Text>
            <TextInput style={styles.input} value={corrValue} onChangeText={setCorrValue} placeholder={t('med.newValue')} placeholderTextColor={palette.textMuted} />
            <Text style={styles.label}>{t('med.reasonForChange')}</Text>
            <TextInput style={styles.textArea} value={corrReason} onChangeText={setCorrReason} placeholder={t('med.reasonForChange')} placeholderTextColor={palette.textMuted} multiline />
            <View style={styles.modalActions}>
              ...
              <TouchableOpacity style={styles.saveButton} onPress={() => void correct()} disabled={busy}>
                {busy ? <ActivityIndicator color="#fff" /> : <Text style={styles.saveText}>{t('med.saveDraft')}</Text>}
              </TouchableOpacity>
            </View>
          </View>
        </View>
