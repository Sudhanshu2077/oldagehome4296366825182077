...
export default function VisitBookNewScreen() {
  ...
  async function saveDraft() {
    ...
    setError(null);
    try {
      await api.post('/visit-book', { entryDate: entryDate || null, officerName, officerPost, remark });
      router.replace('/visit-book/list');
    } catch (err) {
  ...
  async function saveAndSubmit() {
    ...
    setError(null);
    try {
      if (!entryDate || !officerName.trim()) {
        setError(t('visitbook.required'));
        return;
      }
      const res = await api.post('/visit-book', { entryDate, officerName, officerPost, remark });
    ...
    <View style={styles.container}>
      <ScrollView style={styles.scroll}>
        ...
        <View style={styles.field}>
          <Text style={styles.label}>{t('visitbook.date')} (YYYY-MM-DD)</Text>
        ...
        <View style={styles.field}>
          ...
          <TextInput style={styles.input} value={officerName} onChangeText={setOfficerName} placeholder={t('visitbook.officerName')} placeholderTextColor={palette.textMuted} />
        </View>
        <View style={styles.field}>
          <Text style={styles.label}>{t('visitbook.officerPost')}</Text>
          <TextInput style={styles.input} value={officerPost} onChangeText={setOfficerPost} placeholder={t('visitbook.officerPost')} placeholderTextColor={palette.textMuted} />
        </View>
        <View style={styles.field}>
          <Text style={styles.label}>{t('visitbook.remark')}</Text>
          <TextInput style={styles.textArea} value={remark} onChangeText={setRemark} placeholder={t('visitbook.remark')} placeholderTextColor={palette.textMuted} multiline />
        </View>
        <View style={styles.actions}>
          <TouchableOpacity style={styles.draftButton} onPress={() => void saveDraft()} disabled={busy}>
            {busy ? <ActivityIndicator color={palette.primary} /> : <Text style={styles.draftButtonText}>{t('visitbook.saveDraft')}</Text>}
          </TouchableOpacity>
          <TouchableOpacity style={styles.submitButton} onPress={() => void saveAndSubmit()} disabled={busy}>
            {busy ? <ActivityIndicator color="#fff" /> : <Text style={styles.submitText}>{t('visitbook.submit')}</Text>}
          </TouchableOpacity>
        </View>
      </ScrollView>
    </View>
